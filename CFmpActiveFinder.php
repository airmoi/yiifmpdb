...
/**
 * CFmpActiveFinder implements eager loading and lazy loading of related active records.
 *
...
class CFmpActiveFinder extends CComponent
{
	...
	public function __construct($model,$with)
	{
		...
		$this->_joinTree=new CFmpJoinElement($this,$model);
	 ...
	 * @param CFmpJoinElement $parent the parent tree node
	 ...
	 * @throws CDbException if given parent tree node is an instance of {@link CFmpStatElement}
	...
	{
		if($parent instanceof CFmpStatElement)
		...
		if(is_string($with))
		{
			...
			if($relation instanceof CStatRelation)
				return new CFmpStatElement($this,$relation,$parent);
			else
			{
				if(isset($parent->children[$with]))
				{
					...
					$element=new CFmpJoinElement($this,$relation,$parent,++$this->_joinCount);
...
/**
 * CFmpJoinElement represents a tree node in the join tree created by {@link CFmpActiveFinder}.
 *
 * @author Qiang Xue <qiang.xue@gmail.com>
 * @package system.db.ar
 * @since 1.0
 */
class CFmpJoinElement
{
	/**
	...
	/**
	 * Constructor.
	 * @param CFmpActiveFinder $finder the finder
	...
	public function find($criteria=null)
	{
		...
		{
			$query=new CFmpJoinQuery($this,$criteria);
		...
		{
			$query=new CFmpJoinQuery($this->_parent);
			$this->_joined=true;
			$query->join($this);
			$this->buildQuery($query);
	 ...
	 * @param CFmpJoinQuery $query represents a JOIN SQL statements
	...
	private function applyLazyCondition($query,$record)
	{
		...
		if($this->relation instanceof CManyManyRelation)
		{
			...
			foreach($fks as $i=>$fk)
			{
				if(isset($joinTable->foreignKeys[$fk]))  // FK defined
				{
					...
					{
                                                $val = $joinTable->getColumn($fk)->typecast($record->$pk);
						$parentCondition[$pk]=$joinAlias.'.'.$schema->quoteColumnName($fk).'='.$val;
				...
				foreach($fks as $i=>$fk)
				{
					if($i<count($parent->_table->primaryKey))
					{
						$pk=is_array($parent->_table->primaryKey) ? $parent->_table->primaryKey[$i] : $parent->_table->primaryKey;
                                                $val = $parent->_table->getColumn($pk)->typecast($record->$pk);
						$parentCondition[$pk]=$joinAlias.'.'.$schema->quoteColumnName($fk).'='.$val;
						//$params[':ypl'.$count]=$record->$pk;
						$count++;
					}
		...
		else
		{
			...
			foreach($params as $name=>$value)
			{
                            $value = $element->_table->getColumn($name)->typecast($value);
				$query->conditions[]=$prefix.$schema->quoteColumnName($name).'='.$value;
                                /*$test = $element->_table->columns->$pk;
	 ...
	 * @param CFmpJoinQuery $query the query being built up
	 */
	 ...
	 * @param CFmpJoinQuery $query the query to be executed.
	 */
	 ...
	 * @param CFmpJoinQuery $query the query executed
	 ...
	 * @param CFmpJoinElement $fke the join element containing foreign keys
	 * @param array $fks the foreign keys
	 * @param CFmpJoinElement $pke the join element contains primary keys
	 * @param CFmpJoinElement $parent the parent join element
...
/**
 * CFmpJoinQuery represents a JOIN SQL statement.
 *
 * @author Qiang Xue <qiang.xue@gmail.com>
 * @package system.db.ar
 * @since 1.0
...
class CFmpJoinQuery
{
	...
	/**
	 * Constructor.
	 * @param CFmpJoinElement $joinElement The root join tree.
	 * @param CDbCriteria $criteria the query criteria
	 */
	 ...
	 * @param CFmpJoinElement $element the element to be joined
	 */
...
/**
 * CFmpStatElement represents STAT join element for {@link CFmpActiveFinder}.
 *
 * @author Qiang Xue <qiang.xue@gmail.com>
 * @package system.db.ar
 */
class CFmpStatElement
{
	/**
	 * @var CActiveRelation the relation represented by this tree node
	 */
	 ...
	 * @param CFmpJoinElement $parent the join element owning this STAT element
	 */
